...
2. In order to execute this app in command line, compile the Task.java once and  change the class path in script files.

   **On Windows:**
   In task.bat file change
   ```
   java -cp /Users/dhanushn/Downloads/java/ Task %1 %2 to java -cp /path/to/class file/ Task %1 %2
   ```
   **On Linux/Mac:**
   In task.sh file change
   ```
   Change java -cp /Users/dhanushn/Downloads/java/ Task "$@"  to  java -cp /path/to/class file/ Task "$@"
...
2. The app  reads from and write to a task.txt text file. Each task occupies a single line in this file. Each line in the file will be in this format :
   ...
   ```
   1 Buy chocolate
   2 Complete the proposal writing
   ```
...
5. If two task have the same priority, the task that was added first will be displayed first.
   ...
   The application will open the files task.txt and completed.txt from where the app is run, and not where the app is located.
    For example, if we invoke the app like this:
...
```
$ ./task ls
1. change light bulb [5]
2. water the plants [7]
```
...
Attempting to delete a non-existent item wil display an error message.
